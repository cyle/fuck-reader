...
feed_urls.each { |feed_info|
	...
	begin
		...
		parsed.entries.each do |entry|
			...
			puts "Entry title: " + entry.title.to_ascii unless entry.title.nil?
			...
			if entry.title.nil?
				...
				dbchk_query = dbchk_query + "post_title='" + dbclient.escape(entry.title.to_ascii).slice(0..249) + "'"
			end
			...
			unless dbchkresults.count > 0
				...
				if entry.title.nil?
					...
					entry_title_db = "'" + dbclient.escape(entry.title.to_ascii) + "'"
				end
