...
if (isset($_GET['all']) && trim($_GET['all']) == 'yup') {
	...
	$get_all_unread_post_ids = $mysqli->query("SELECT post_id FROM posts WHERE ts < NOW() AND post_id NOT IN (SELECT post_id FROM users_read_posts WHERE user_id=$current_user_id)");
	...
	$get_feeds_unread_post_ids = $mysqli->query("SELECT post_id FROM posts WHERE ts < NOW() AND feed_id=$feed_id AND post_id NOT IN (SELECT post_id FROM users_read_posts WHERE user_id=$current_user_id)");
	// mark all of those posts as read
